...
export function destinationAfterSignIn(requested: string, isAdmin: boolean): string {
...
async function readSessionFromApi(): Promise<SessionShape> {
  try {
...
export async function waitForClientSession(maxAttempts = 20, delayMs = 200): Promise<SessionShape> {
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    const session = (await getSession()) ?? (await readSessionFromApi());
    if (session?.user) {
      return session;
    }
...
export default function SignInForm({ callbackUrlParam }: { callbackUrlParam: string | null }) {
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [error, setError] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const requestedCallbackUrl = safeCallbackUrl(callbackUrlParam);
  ...
  const handleGoogleSignIn = () => {
    signIn('google', { callbackUrl: requestedCallbackUrl });
  };
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="flex items-center justify-between">
              <div className="text-sm">
                <Link href="/auth/forgot" className="font-medium text-gray-900 hover:text-gray-700">
