// filepath: src/ui/components/commerce/RecentlyViewed.tsx
'use client';
import React, { useState, useEffect } from 'react';
import Link from 'next/link';
import Image from 'next/image';
import { useSession } from 'next-auth/react';
import { Product, getProductById } from '@domain/shop';
interface RecentlyViewedProps { currentProductId?: string; maxItems?: number; className?: string; }
interface ViewedProduct { productId: string; viewedAt: number; }
export function RecentlyViewed({ currentProductId, maxItems = 6, className = "" }: RecentlyViewedProps) {
    ...
    const loadRecentlyViewed = async () => {
      try {
        ...
        if (session?.user?.id) {
          ...
          if (data.success) setRecentlyViewed(data.products);
        } else {
          ...
          <Link key={product.id} href={`/shop/${product.id}`} className="group">
            <div className="bg-white rounded-lg border border-gray-200 overflow-hidden hover:shadow-md transition-shadow duration-200">
              <div className="aspect-square relative overflow-hidden">
                <Image src={product.images.thumbnail} alt={product.title} fill className="object-cover group-hover:scale-105 transition-transform duration-200" sizes="(max-width: 768px) 50vw, (max-width: 1024px) 33vw, 16vw" />
              </div>
              <div className="p-3">
