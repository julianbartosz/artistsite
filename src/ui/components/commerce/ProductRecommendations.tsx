// filepath: src/ui/components/commerce/ProductRecommendations.tsx
'use client';
    ...
    <Link href={`/shop/${product.id}`} className="group">
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow duration-200">
        <div className="aspect-square relative overflow-hidden">
          <Image src={product.images.thumbnail} alt={product.title} fill className="object-cover group-hover:scale-105 transition-transform duration-200" />
        </div>
        <div className="p-4">
          ...
          <div className="flex items-center justify-between">
            ...
            {product.category && (<span className="text-xs text-gray-500 capitalize">{product.category}</span>)}
          </div>
        </div>
      </div>
    </Link>
...
function getRecommendationTitle(type: string): string {
  switch (type) {
    case 'similar': return 'Similar Artworks';
    case 'frequently_bought_together': return 'Frequently Bought Together';
    case 'viewed_together': return 'People Also Viewed';
