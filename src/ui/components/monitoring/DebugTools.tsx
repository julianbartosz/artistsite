// filepath: src/ui/components/monitoring/DebugTools.tsx
'use client'
...
export function DebugTools() {
  ...
  const handleTestError = () => {
    try { throw new Error('Test error for debugging') } catch (error) {
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="space-y-4">
            <button onClick={handleTestLog} className="w-full bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded transition-colors">Test Console Log</button>
            <button onClick={handleTestError} className="w-full bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded transition-colors">Trigger Test Error</button>
            <button onClick={handleReload} className="w-full bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded transition-colors">Reload Page</button>
          </div>
        </div>
        <div className="bg-white rounded-lg shadow p-6">
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={clearLogs} className="text-sm bg-gray-500 hover:bg-gray-600 text-white px-3 py-1 rounded">Clear</button>
        </div>
        <div className="bg-gray-50 rounded p-4 min-h-32 max-h-64 overflow-y-auto">
          {logs.length > 0 ? (
            <div className="space-y-2">
              {logs.map((log, index) => (
                <div key={index} className="text-sm font-mono text-gray-700 border-b border-gray-200 pb-1">{log}</div>
              ))}
            </div>
          ) : (
